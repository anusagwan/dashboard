...
function useInstance(instance) {
  ...
  ensurePluginOrder(plugins, ["useColumnOrder", "useColumnVisibility", "useFilters", "useSortBy"], "useExportData"); // Adding `canExport` & `exportValue` meta data
  ...
  allColumns.forEach(column => {
    ...
    const canExport = accessor ? getFirstDefined(column.disableExport === true ? false : undefined, disableExport === true ? false : undefined, true) : false;
  ...
  const exportData = React.useCallback((fileType, all = false) => {
    ...
    let exportableRows = (all ? initialRows : rows).map(row => {
      return exportableColumns.map(col => {
        const {
          getCellExportValue = defaultGetCellExportValue
        } = col;
